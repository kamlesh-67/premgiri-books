...
export default function NewPurchaseOrderPage() {
  ...
  // Fetch suppliers (LIABILITY nature ledgers — creditors / payables)
  const { data: suppliers = [] } = useQuery<LedgerOption[]>({
    queryKey: ['ledgers-liability'],
    queryFn: async () => {
      const res = await fetch('/api/v1/masters/ledgers?nature=LIABILITY')
  ...
  const mutation = useMutation({
    ...
    onSuccess: (newOrder) => {
      ...
      toast.success(`Purchase order ${newOrder.orderNo} created`)
      router.push(`/purchase-order/${newOrder.id}`)
    },
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      <PageHeader
        ...
        subtitle="Create a purchase commitment with a supplier"
      />
