...
export async function GET(request: Request) {
  ...
  const data = await getBalanceSheet(companyId, fy)
  const buffer = await exportToExcel(data, 'balance-sheet')

  return new Response(new Uint8Array(buffer), {
    headers: {
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': `attachment; filename="balance-sheet-${fy}.xlsx"`,
    },
  })
}
