...
export default function Journal() {
  const rows = useCollection("journals");
  ...
  return (
    <div>
      ...
      />
      <SectionCard>
        <DataTable columns={columns} rows={rows} rowKey={(r) => r.id} empty="No journals yet." />
      </SectionCard>
      <JournalForm open={open} onOpenChange={setOpen} initial={editing} />
      <ConfirmDelete open={!!toDelete} onOpenChange={(o) => !o && setToDelete(null)}
        title={`Delete ${toDelete?.number}?`}
        onConfirm={() => { if (toDelete) { remove("journals", toDelete.id); toast.success(`${toDelete.number} deleted`); } setToDelete(null); }} />
    </div>
  );
}
