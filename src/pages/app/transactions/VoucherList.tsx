...
const columns: Column<VoucherRow>[] = [
  { key: "number", header: "Voucher #", cell: (r) => <Link to={`/vouchers/${r.id}`} className="font-medium text-primary hover:underline">{r.number}</Link> },
  ...
  { key: "status", header: "Status", cell: (r) => <StatusBadge status={r.status} /> },
  {
    ...
    cell: (r) => <Link to={`/vouchers/${r.id}`} className="inline-flex rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"><Eye className="h-4 w-4" /></Link>,
  },
];
